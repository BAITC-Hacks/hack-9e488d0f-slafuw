...
**Актуальная архитектура агента и провайдеров — [PROVIDERS.md](PROVIDERS.md).**
Ниже сохранено описание исходного проверочного ядра и проектных решений.
Разделы с пометкой «план» — история проектирования; реализация сейчас использует
OpenAI Responses, NVIDIA API и SQLite, как описано в актуальной инструкции.
...
| LLM-извлечение и multilingual embeddings | Реализованы OpenAI и NVIDIA клиенты; live-проверка требует ключей |
| Краткие контрастные объяснения на структурированных признаках | Выбор fact_id/evidence_id + серверная сборка |
...
Доменное ядро не зависит от HTTP и браузера. Серверный слой использует SQLite для сессий,
результатов и журнала; сетевые обращения включаются серверной конфигурацией провайдеров.
Локальный HTTP-адаптер можно заменить, сохранив инструменты `EventMatchService` и тесты.
