...
def handler_for(catalog, service=None, agent=None):
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            try:
                ...
                if self.path == "/calendar/event":
                    ...
                    if not required.issubset(raw):
                        raise ValueError(
                            "Нужны title, start_time и end_time"
                        )
                ...
                # ===== CHAT =====
                    ...
                    if (
                        set(raw) - {"message", "session_id"}
                        or "message" not in raw
                    ):
                        raise ValueError(
                            "Нужны message и опциональный session_id"
                        )
                ...
                self.respond(
                    503,
                    {
                        ...
                        "message": (
                            "Технически невозможно выполнить запрос"
                        ),
                    },
                )
                return
            ...
            self.respond(
                503
                if result.get("status") == "technical_error"
                else 200,
                result,
            )
